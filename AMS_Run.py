import tkinter as tk #ui function 
from tkinter import *
import ICA #import for ica lib  algoritm 
import cv2 #used for access the camera on pc 
import csv #sheets 
import os
import numpy as np #model
from PIL import Image,ImageTk  #for background image to interface 
...
x=0.9379
y=0.2342
W = ICA.fastIca(x,y) #ica algoritm function 
#####Window is our Main frame of system
window = tk.Tk()
window.title("FAMS-Face Recognition Based Model")

window.geometry('1280x720')
window.configure(background='#a8d5e5')#background color

####GUI for manually fill attendance (in case 3ayzeen system)

def manually_fill():
    global sb
    sb = tk.Tk() #n3rf new interface 
    sb.iconbitmap('AMS.ico')
    sb.title("Enter subject name...")
    sb.geometry('580x320')
    sb.configure(background='snow')

    def err_screen_for_subject():#function for if there is error in subject 
    ...
    def fill_attendance():# save attendece in database
        ts = time.time()
        ...
        global subb #subject name
        subb=SUB_ENTRY.get() #subjectb from interface
        DB_table_name = str(subb + "_" + Date + "_Time_" + Hour + "_" + Minute + "_" + Second)

        import pymysql.connections  ###Connect to the database
        try:
            global cursor
            connection = pymysql.connect(host='localhost', user='root', password='', db='facereco')
        ...
        if subb=='':
            ...
            sb.destroy()# close the screen als8era
            MFW = tk.Tk()
            ...
            def err_screen1():
                ...
                Label(errsc2, text='Please enter Student & Enrollment!!!', fg='red', bg='white',
                      font=('times', 16, ' bold ')).pack() #improove alfont
            ...
            def testVal(inStr, acttyp):
                if acttyp == '1':  # insert
                    if not inStr.isdigit():
                        return False
                return True#constrqin just insert didgys
            ...
            def create_csv():
                ...
                csv_name='C:/ICA/ICA/Attendance/Manually Attendance/'+DB_table_name+'.csv'
            ...
            def attf():
                import subprocess
                subprocess.Popen(r'explorer /select,"C:/ICA/ICA/Attendance/Manually Attendance/-------Check atttendance-------"')
...
def take_img():
    ...
    if l1 == '':
        ...
        try:
            ...
            Notifcv2tion.configure(text=res, bg="SpringGreen3", width=50, font=('times', 18, 'bold'))
            Notifcv2tion.place(x=250, y=400)
        except FileExistsError as F:
            f = 'Student Data already exists'
            Notifcv2tion.configure(text=f, bg="Red", width=21)
            Notifcv2tion.place(x=450, y=400)
...
def subjectchoose():
        ...
        if time.time() < future:
            if sub == '':
                ...
                try:
                    ...
                    Notifcv2.configure(text=e, bg="red", fg="black", width=33, font=('times', 15, 'bold'))
                    Notifcv2.place(x=20, y=250)
                ...
                Notifcv2.configure(text=M, bg="Green", fg="white", width=33, font=('times', 15, 'bold'))
                Notifcv2.place(x=20, y=250)
    ...
    Notifcv2 = tk.Label(windo, text="Attendance filled Successfully", bg="Green", fg="white", width=33,
                            height=2, font=('times', 15, 'bold'))

    def Attf():
        import subprocess
        subprocess.Popen(r'explorer /select,"C:/ICA/ICA/Attendance/-------Check atttendance-------"')
    ...
    def log_in():
        ...
        if username == 'admin' :
            if password == 'admin':
                ...
                cs = 'C:/ICA/ICA/StudentDetails/StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
...
def trainimg():
    ...
    try:
        ...
        Notifcv2tion.configure(text=l, bg="SpringGreen3", width=50, font=('times', 18, 'bold'))
        Notifcv2tion.place(x=350, y=400)
    ...
    try:
        ...
        Notifcv2tion.configure(text=q, bg="SpringGreen3", width=50, font=('times', 18, 'bold'))
        Notifcv2tion.place(x=350, y=400)
...
message = tk.Label(window, text="Face-Recognition-Based-Attendance Model", bg="#C7A8A8", fg="black", width=50,
                   height=3, font=('times', 30, 'italic bold '))

message.place(x=80, y=20)

Notifcv2tion = tk.Label(window, text="All things good", bg="Green", fg="white", width=15,
                      height=3, font=('times', 17, 'bold'))

lbl = tk.Label(window, text="Enter Enrollment", width=20, height=2, fg="white", bg="green", font=('times', 15, ' bold '))
...
takeImg = tk.Button(window, text="Take Images",command=take_img,fg="black"  ,bg="gray"  ,width=20  ,height=3, activebackground = "Red" ,font=('times', 15, ' bold '))
takeImg.place(x=90, y=500)

trainImg = tk.Button(window, text="Train Images",fg="black",command=trainimg ,bg="gray"  ,width=20  ,height=3, activebackground = "Red" ,font=('times', 15, ' bold '))
trainImg.place(x=500, y=500)

FA = tk.Button(window, text="Automatic Attendace",fg="black",command=subjectchoose  ,bg="gray"  ,width=20  ,height=3, activebackground = "Red" ,font=('times', 15, ' bold '))
